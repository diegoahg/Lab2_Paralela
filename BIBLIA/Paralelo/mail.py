...
def envio_mail_paralelo_implicito(largo, pdf, keyword, jumpMax, email, match, n_pages, link, documento):
    ...
    if len(match) > 0:
        return requests.post(
            ...
            files=[
                ("attachment", open("/var/www/html/webParalela/BIBLIA/PDF/" + documento + ".pdf", encoding='ISO-8859-1'))],
            data={"from": "Cluster Marvel<postmaster@sandbox3761d267c8184fa2afb562ff83f8881e.mailgun.org>",
                  "to": "Usuario <" + email + ">",
                  "subject": "Resultados Bible Code - PARALELO IMPLICITO",
                  "text":
...
def envio_mail_paralelo_explicito(largo, pdf, keyword, jumpMax, email, match, patron_corto, n_pages, link, documento):
    ...
    if len(match) > 0:
        return requests.post(
            ...
            data={"from": "Cluster Marvel<postmaster@sandbox3761d267c8184fa2afb562ff83f8881e.mailgun.org>",
                  "to": "Usuario <" + email + ">",
                  "subject": "Resultados Bible Code - PARALELO EXPLICITO",
                  "text":
